...
class DatabaseWriter:
    ...
    def insert_telemetry(self, recorded_at: str, voltage: float, temp: float, source_file: str):
        ...
        try:
            with self.conn.cursor() as cur:
                ...
                cur.execute(query, (recorded_at, voltage, temp, source_file))
                self.conn.commit()
...
class TelemetryService:
    ...
    def generate_and_store(self):
        ...
        try:
            ...
            filepath = os.path.join(self.config.csv_out_dir, filename)
            ...
            with open(filepath, 'w', newline='') as csvfile:
                ...
                writer.writerow([recorded_at, f'{voltage:.2f}', f'{temp:.2f}', filename])
            
            logger.info(f"CSV written: {filepath}")
            
            # Insert into database
            self.db_writer.insert_telemetry(recorded_at, voltage, temp, filename)
