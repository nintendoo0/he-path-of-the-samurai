# 🚀 Быстрый старт проекта "Кассиопея"

## Шаг 1: Запуск Docker Desktop
Убедитесь, что Docker Desktop запущен.

## Шаг 2: Запуск проекта

### Полный запуск (рекомендуется)
```powershell
cd d:\he-path-of-the-samurai
docker-compose up --build -d
```

⏱️ **Время сборки:** 10-15 минут (Rust компилируется долго)

### Просмотр логов во время сборки
```powershell
docker-compose logs -f
```

## Шаг 3: Проверка статуса

```powershell
# Проверить статус всех контейнеров
docker-compose ps

# Должны быть запущены:
# - iss_db (PostgreSQL)
# - rust_iss (Rust API)  
# - php (PHP-FPM)
# - nginx (Web сервер)
# - python_legacy (Telemetry)
```

## Шаг 4: Доступ к приложению

После успешного запуска откройте в браузере:

- 🌐 **Главный дашборд**: http://localhost:8080/dashboard
- 🛰️ **ISS Tracker**: http://localhost:8080/iss
- 📊 **OSDR Data**: http://localhost:8080/osdr
- ⚡ **Rust API**: http://localhost:8081/health

## Проверка работоспособности

### 1. Проверка Rust API
```powershell
curl http://localhost:8081/health
# Должен вернуть: {"status":"ok","now":"2025-12-09T..."}
```

### 2. Проверка ISS данных
```powershell
curl http://localhost:8081/last
# Вернёт последние данные о позиции ISS
```

### 3. Проверка базы данных
```powershell
docker-compose exec db psql -U monouser -d monolith -c "SELECT count(*) FROM iss_fetch_log;"
```

### 4. Проверка Python Legacy
```powershell
docker-compose logs python_legacy --tail=10
# Должны быть записи: "Inserted telemetry record: ..."
```

## Решение проблем

### Если Rust долго собирается
Это нормально! Первая сборка Rust может занять 10-15 минут.

Проверьте прогресс:
```powershell
docker-compose logs rust_iss
```

### Если PHP собирается очень долго
PHP контейнер устанавливает компиляторы (llvm, clang) для расширения PostgreSQL.
Это может занять 3-5 минут при первой сборке.

### Ошибка "host not found in upstream php"
Это значит PHP контейнер не запустился. Проверьте:
```powershell
docker-compose logs php
```

Если видите ошибки с `$'\r': command not found` — это проблема line endings.
Исправление уже применено в Dockerfile (автоконвертация CRLF→LF).

### Если порт уже занят
Измените порты в `docker-compose.yml`:
```yaml
nginx:
  ports:
    - "8080:80"  # Измените 8080 на другой порт
```

### Если ошибка сборки
Очистите и пересоберите:
```powershell
docker-compose down -v
docker-compose up --build
```

## Остановка проекта

### Остановить с сохранением данных
```powershell
docker-compose stop
```

### Остановить и удалить контейнеры
```powershell
docker-compose down
```

### Полная очистка (включая volumes)
```powershell
docker-compose down -v
```

## Полезные команды

```powershell
# Перезапуск конкретного сервиса
docker-compose restart rust_iss

# Просмотр логов конкретного сервиса
docker-compose logs -f nginx

# Подключение к контейнеру
docker-compose exec rust_iss sh
docker-compose exec db psql -U monouser -d monolith

# Пересборка конкретного сервиса
docker-compose up --build rust_iss -d

# Статистика ресурсов
docker stats
```

## API Endpoints

### Rust API (http://localhost:8081)
- `GET /health` - Health check
- `GET /last` - Последняя позиция ISS
- `GET /iss/trend` - Тренд движения ISS
- `GET /osdr/list` - Список OSDR датасетов
- `GET /osdr/sync` - Синхронизация OSDR
- `GET /space/apod/latest` - Астрономическое фото дня
- `GET /space/summary` - Сводка всех данных

### Laravel (http://localhost:8080)
- `GET /dashboard` - Главный дашборд
- `GET /iss` - Страница ISS
- `GET /osdr` - Страница OSDR
- `GET /api/jwst/feed` - JWST галерея
- `GET /api/astro/events` - Астрономические события

## Переменные окружения

Основные настройки в файле `.env`:
- `NASA_API_KEY` - ключ NASA API
- `DATABASE_URL` - подключение к PostgreSQL
- Интервалы обновления (`*_EVERY_SECONDS`)

### Настройка внешних API

Для полной функциональности сайта вам может потребоваться настроить дополнительные API.

См. подробную инструкцию в файле [API_SETUP.md](API_SETUP.md).

**Краткая справка:**
- ✅ NASA API - уже настроен (demo ключ)
- ✅ JWST API - не требует ключа
- ✅ ISS Tracking - не требует ключа  
- ⚠️ AstronomyAPI - опционально, требует регистрации на https://astronomyapi.com

**Важно:** Сайт полностью работает и без AstronomyAPI. Секция астрономических событий просто будет пустой до настройки ключей.

## Структура проекта

```
.
├── services/
│   ├── rust-iss/        # Rust API сервис
│   ├── php-web/         # Laravel веб-приложение
│   ├── python-legacy/   # Python телеметрия
│   └── pascal-legacy/   # Старый Pascal (архив)
├── db/                  # SQL инициализация
├── docker-compose.yml   # Конфигурация Docker
├── .env                 # Переменные окружения
└── INSTALL.md          # Подробная инструкция
```

## Документация

- 📖 [REFACTORING_REPORT.md](REFACTORING_REPORT.md) - Полный отчёт по рефакторингу
- 📋 [CHANGELOG.md](CHANGELOG.md) - Список изменений
- 📚 [README.md](README.md) - Общее описание проекта

---

**Версия:** 1.0  
**Дата:** 9 декабря 2025 г.
